...
const ManagersList = () => {
  ...
  const fetchManagers = async () => {
    try {
      ...
      toast.error(error.message || "An error occurred while fetching managers");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDelete = async (managerId, managerName) => {
    if (
      window.confirm(
        `Are you sure you want to remove ${managerName} from the system?`
      )
    ) {
      try {
  ...
  return (
    <div className="p-6 min-h-screen bg-gray-50">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-800">Management Staff</h1>
          <p className="text-gray-500 mt-1">
            Manage your hospital's department managers
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {Array.isArray(managers) && managers.length > 0 ? (
            ...
            >
              ...
              <div className="p-5">
                ...
                <div className="mt-3 text-sm text-gray-500 line-clamp-2">
                  {manager.ABOUT || "No description available."}
                </div>
                ...
                <div className="flex gap-3 mt-4">
                  <button
                    onClick={() =>
                      handleDelete(manager.MANAGER_ID, manager.NAME)
                    }
          ...
          <div className="col-span-full text-center py-12 bg-white rounded-xl border border-gray-100">
            ...
            <p className="text-gray-600 text-lg">No managers available</p>
            <p className="text-gray-400 text-sm mt-1">
              Add new managers to see them here
            </p>
          </div>
        )}
      </div>
