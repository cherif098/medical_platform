...
const AddSecretary = () => {
  const [secretaryImg, setSecretaryImg] = useState(false);
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      ...
      formData.append("IMAGE", secretaryImg);
      ...
      const { data } = await axios.post(
        backendUrl + "/api/admin/add-secretary",
        formData,
        {
          headers: { aToken },
        }
      ...
      if (data && data.success) {
        ...
        toast.error(data.message || "Failed to add secretary");
      }
    } catch (err) {
      console.error("Error details:", err);
      toast.error(err.response?.data?.error || "Failed to add secretary");
    }
  };
  ...
  return (
    <div className="p-6 max-w-[1200px] mx-auto">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-800">Add New Secretary</h1>
        <p className="text-gray-500 mt-1">
          Fill in the details to add a new administrative professional
        </p>
      </div>
      ...
      >
        ...
        <div className="p-6 border-b">
          ...
          <div className="flex items-center gap-6">
            ...
            >
              <div className="w-24 h-24 rounded-full overflow-hidden bg-gray-50 border-2 border-dashed border-gray-200 flex items-center justify-center group-hover:border-indigo-500 transition-colors">
                <img
                  ...
                  src={
                    secretaryImg
                      ? URL.createObjectURL(secretaryImg)
                      : assets.upload_area
                  }
                  alt=""
                />
            ...
            <input
              onChange={(e) => setSecretaryImg(e.target.files[0])}
              type="file"
              name="secretary-img"
              id="secretary-img"
              className="hidden"
            ...
            <div className="text-sm text-gray-500">
              <p className="font-medium">Upload secretary photo</p>
        ...
        <div className="p-6 border-t">
          ...
          <textarea
            ...
            rows="4"
            placeholder="Write a brief description about the secretary's background and expertise..."
            required
          />
        </div>
