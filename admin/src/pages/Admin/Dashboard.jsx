        ...
        <div className='pt-4 border border-t-0'>
        {
          dashData.latestAppointments.map((item, index) => (
            <div className='flex items-center px-6 py-3 gap-3' key={index}>
              <img className='w-10 h-10 rounded-full' src={item.DOCTORIMAGE} alt="" />
              <div className='flex-1 text-sm'>
                <p className='text-gray-800 font-medium'>{item.DOCTORNAME}</p>
                <p className='text-gray-600'>{slotDateFormat(item.SLOT_DATE)} à {convertTimeToAmPm(item.SLOT_TIME)}</p>
              </div>
            </div>
          ))}
